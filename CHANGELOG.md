...
- DNS L7: `aggregator_resolve_*` → `alligator_dns_rr_info` / `alligator_dns_rr_count` / `alligator_dns_soa_*` / `alligator_dns_resolve_duration_seconds`. Resolver quantiles stay: `alligator_dns_{read,write,response}_duration_seconds` with `quantile` (values in seconds, was `resolver_*_time_mcs_quantile` microseconds).
- ICMP: `aggregator_packet_*` → `alligator_icmp_replies` / `_losses` / `_duplicates` / `_reply_ratio` / `_loss_ratio` / `_replies_total` / `_losses_total` / `_echoes_total`. Dropped `aggregator_read_time` (use `alligator_icmp_rtt_seconds`).
- Cluster lock gauges: `aggregator_cluster_*` → `alligator_cluster_*`.
- Rename `alligator_connect_ok_total` (counter) to `alligator_session_connect_ok` (gauge): last backend connection success (1) or failure (0).
- PostgreSQL libpq poll path increments `alligator_session_connects_total` on each connect success or failure (same labels as `alligator_session_connect_ok`).
- Probe/blackbox metrics no longer emit blackbox_exporter `probe_*` aliases when a native series already exists. HTTP status/body/cert/phase timings stay on `alligator_http_response_status_code`, `alligator_http_response_body_bytes`, `x509_cert_not_after`, and `alligator_session_duration_seconds`. ICMP last-echo RTT is `alligator_icmp_rtt_seconds`; hop-limit is one series `alligator_icmp_reply_hop_limit`. DNS resolve is `alligator_dns_resolve_duration_seconds`. Timeout/IP family gauges are `alligator_probe_timeout_seconds` / `alligator_probe_ip_protocol` from the resolved socket family, not a hostname colon heuristic. Kept unique: `probe_success`, `probe_failed_due_to_regex`, `probe_expect_info`.
- Probe HTTP `body_file` reads the request body from a file on each `/probe` kick (blackbox_exporter-compatible). Inline `body` and `body_file` together is invalid; `body_file` wins. A missing file returns HTTP 400 and does not kick the probe.
- ICMPv6 `alligator_icmp_reply_hop_limit` is taken from `recvmsg` ancillary data (`IPV6_RECVHOPLIMIT` / `IPV6_HOPLIMIT`). IPv4 still uses the IP header TTL (with the same cmsg helper as fallback).
- ICMP `aggregator_packet_received_percent` / `aggregator_packet_loss_percent` renamed to `alligator_icmp_reply_ratio` / `alligator_icmp_loss_ratio` (values stay 0–1).
...
- Fix: shared `dns udp://… bind_address=<port>` no longer SIGSEGVs in `uv__udp_recvmsg` (jump to NULL). libuv `alloc_buffer` treated the shared bind object as a `context_arg` and could zero `recv_cb`; recv is now started before `uv_udp_send`, and empty `nread=0` datagrams are ignored instead of tearing the socket down.
- Fix: filetailer `period=` no longer calls `uv_timer_set_repeat` on a NULL timer. The period timer is created when missing rather than only on the first crawl (`close_counter==0`).
- Explicit `dns udp://... resolve=<domain>` probes export `name` on `alligator_dns_read_duration_seconds` / `alligator_dns_write_duration_seconds` / `alligator_dns_response_duration_seconds` so timing is per domain, not only per nameserver.
