...
## [unreleased]
- Session I/O metrics renamed to `alligator_session_*` with durations in seconds and a `stage` label (`connect`, `write`, `read`, `tls_handshake`, `tls_write`, `tls_read`, `shutdown`, `total`). Parser result is `alligator_parser_ok` / `alligator_parser_duration_seconds`. Filetailer opens are `alligator_filetailer_opens_total`.
