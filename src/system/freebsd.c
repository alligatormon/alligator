...
void get_swap()
{
	...
	metric_labels_add_lbl("swap_usage", &usedswap, ALLIGATOR_DATATYPE_INT, 0, "type", "usage");
	metric_labels_add_lbl("swap_usage", &totalswap, ALLIGATOR_DATATYPE_INT, 0, "type", "total");
	metric_labels_add_lbl("swap_usage", &availswap, ALLIGATOR_DATATYPE_INT, 0, "type", "avail");
	kvm_close(kd);
}
void get_disk()
{
	...
	for (int i = 0; i < num_mounts; i++) {
		...
		int one = 1;
		metric_labels_add_lbl2("disk_usage", &avail, ALLIGATOR_DATATYPE_INT, 0, "mountpoint", mounts[i].f_mntonname, "type", "avail");
		metric_labels_add_lbl2("disk_usage", &total, ALLIGATOR_DATATYPE_INT, 0, "mountpoint", mounts[i].f_mntonname, "type", "total");
		metric_labels_add_lbl2("disk_usage", &used, ALLIGATOR_DATATYPE_INT, 0, "mountpoint", mounts[i].f_mntonname, "type", "used");
...
void get_mem()
{
	...
	metric_labels_add_lbl("memory_usage", &inact_memory, ALLIGATOR_DATATYPE_INT, 0, "type", "inactive");
	metric_labels_add_lbl("memory_usage", &real_memory, ALLIGATOR_DATATYPE_INT, 0, "type", "real");
	metric_labels_add_lbl("memory_usage", &active_memory, ALLIGATOR_DATATYPE_INT, 0, "type", "active");
	metric_labels_add_lbl("memory_usage", &avail_memory, ALLIGATOR_DATATYPE_INT, 0, "type", "available");
	...
	metric_labels_add_auto("vm_faults", &avail_memory, ALLIGATOR_DATATYPE_INT, 0);
	metric_labels_add_auto("io_faults", &avail_memory, ALLIGATOR_DATATYPE_INT, 0);
	metric_labels_add_auto("cores_num", &num_cpu, ALLIGATOR_DATATYPE_INT, 0);
	metric_labels_add_auto("effective_cores_num", &num_cpu, ALLIGATOR_DATATYPE_INT, 0);
}
		...
		if (maxcpu > 0) {
			...
			for (n = 0; n < maxcpu; n ++)
				if (cpu2[n] && cpu1[n])
				{
					...
					metric_labels_add_lbl2("cpu_usage", &total, ALLIGATOR_DATATYPE_DOUBLE, 0, "core", cpuname, "type", "total");
					metric_labels_add_lbl2("cpu_usage", &user, ALLIGATOR_DATATYPE_DOUBLE, 0, "core", cpuname, "type", "user");
					metric_labels_add_lbl2("cpu_usage", &system,  ALLIGATOR_DATATYPE_DOUBLE, 0, "core", cpuname, "type", "system");
					metric_labels_add_lbl2("cpu_usage", &nice, ALLIGATOR_DATATYPE_DOUBLE, 0, "core", cpuname, "type", "nice");
					metric_labels_add_lbl2("cpu_usage", &intr, ALLIGATOR_DATATYPE_DOUBLE, 0, "core", cpuname, "type", "intr");
					metric_labels_add_lbl2("cpu_usage", &idle, ALLIGATOR_DATATYPE_DOUBLE, 0, "core", cpuname, "type", "idle");
				}
			for (n = 0; n < maxcpu; n ++)
			{
...
void get_proc_info()
{
	...
	for (i=0; i<cntp; i++ )
	{
		...
		metric_labels_add_lbl("process_cpu", &val, ALLIGATOR_DATATYPE_INT, 0, "name", proc[i].ki_comm);
		val = proc[i].ki_size;
		metric_labels_add_lbl2("process_memory", &val, ALLIGATOR_DATATYPE_INT, 0, "name", proc[i].ki_comm, "type", "vsz");
		val = proc[i].ki_rssize;
		metric_labels_add_lbl2("process_memory", &val, ALLIGATOR_DATATYPE_INT, 0, "name", proc[i].ki_comm, "type", "rss");
		val = proc[i].ki_cow;
		metric_labels_add_lbl2("process_stats", &val, ALLIGATOR_DATATYPE_INT, 0, "name", proc[i].ki_comm, "type", "COWfaults");
		val = proc[i].ki_numthreads;
		metric_labels_add_lbl2("process_stats", &val, ALLIGATOR_DATATYPE_INT, 0, "name", proc[i].ki_comm, "type", "threads");
	}
	free(proc);
}
...
void get_fd_info()
{
	...
	metric_labels_add_auto("maxsockets", &val, ALLIGATOR_DATATYPE_INT, 0);

	rc = sysctlbyname("kern.ipc.numopensockets", &val, &msz, NULL, 0);
	if (rc < 0){
		perror("sysctlbyname");
	}
	metric_labels_add_auto("opensockets", &val, ALLIGATOR_DATATYPE_INT, 0);

	rc = sysctlbyname("kern.maxproc", &val, &msz, NULL, 0);
	if (rc < 0){
		perror("sysctlbyname");
	}
	metric_labels_add_auto("maxproc", &val, ALLIGATOR_DATATYPE_INT, 0);

	rc = sysctlbyname("kern.maxfiles", &val, &msz, NULL, 0);
	if (rc < 0){
		perror("sysctlbyname");
	}
	metric_labels_add_auto("maxfiles", &val, ALLIGATOR_DATATYPE_INT, 0);

	rc = sysctlbyname("kern.openfiles", &val, &msz, NULL, 0);
	if (rc < 0){
		perror("sysctlbyname");
	}
	metric_labels_add_auto("openfiles", &val, ALLIGATOR_DATATYPE_INT, 0);
}
...
void disk_io_stats() {
	...
	for (int i = 0; i < current.dinfo->numdevs; i++) {
		...
		metric_labels_add_lbl2("disk_io", &bytes_read, ALLIGATOR_DATATYPE_INT, 0, "dev", current.dinfo->devices[i].device_name, "type", "bytes_read");
		metric_labels_add_lbl2("disk_io", &bytes_write, ALLIGATOR_DATATYPE_INT, 0, "dev", current.dinfo->devices[i].device_name, "type", "bytes_write");
		metric_labels_add_lbl2("disk_io", &bytes_free, ALLIGATOR_DATATYPE_INT, 0, "dev", current.dinfo->devices[i].device_name, "type", "bytes_free");
		metric_labels_add_lbl2("disk_io", &transfers_read, ALLIGATOR_DATATYPE_INT, 0, "dev", current.dinfo->devices[i].device_name, "type", "transfers_read");
		metric_labels_add_lbl2("disk_io", &transfers_write, ALLIGATOR_DATATYPE_INT, 0, "dev", current.dinfo->devices[i].device_name, "type", "transfers_write");
		metric_labels_add_lbl2("disk_io", &transfers_free, ALLIGATOR_DATATYPE_INT, 0, "dev", current.dinfo->devices[i].device_name, "type", "transfers_free");
	}

	int64_t disknum = current.dinfo->numdevs;
	metric_labels_add_auto("disk_num", &disknum, ALLIGATOR_DATATYPE_INT, 0);
	free(info);
}
...
# endif
			...
			metric_labels_add_lbl2("if_stat", &ibytes, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "received_bytes");
			metric_labels_add_lbl2("if_stat", &obytes, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "transmit_bytes");
			metric_labels_add_lbl2("if_stat", &ipackets, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "received_packages");
			metric_labels_add_lbl2("if_stat", &opackets, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "transmit_packages");
			metric_labels_add_lbl2("if_stat", &ierrors, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "received_err");
			metric_labels_add_lbl2("if_stat", &oerrors, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "transmit_err");
			metric_labels_add_lbl2("if_stat", &iqdrops, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "received_drop");
# if __FreeBSD__ > 10
			metric_labels_add_lbl2("if_stat", &oqdrops, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "transmit_drop");
# endif
			metric_labels_add_lbl2("if_stat", &imcasts, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "received_multicast");
			metric_labels_add_lbl2("if_stat", &omcasts, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "transmit_multicast");
			metric_labels_add_lbl2("if_stat", &collisions, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "collisions");
			metric_labels_add_lbl2("if_stat", &noproto, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "noproto");
			metric_labels_add_lbl2("if_stat", &link_state, ALLIGATOR_DATATYPE_INT, 0, "ifname", ifa->ifa_name, "type", "link_state");
		}
	}
	freeifaddrs(ifap);
}
