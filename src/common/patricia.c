...
rnode *patricia_insert(patricia_t *tree, uint32_t key, uint32_t mask, void *data, char *s_ip) {
    ...
    if (next) {
        if (node->data) {
            if ((key != 0) && (mask != 0)) {
                glog(L_WARN, "patricia_insert: ACL already contains '%s', replacing entry\n", s_ip);
            }
            free(node->data);
            node->data = data;
            return node;
...
rnode *patricia_insert128(patricia_t *tree, uint128_t key, uint128_t mask, void *data, char *s_ip) {
    ...
    if (next) {
        if (node->data) {
            if ((key != 0) && (mask != 0)) {
                glog(L_WARN, "patricia_insert128: ACL already contains '%s', replacing entry\n", s_ip);
            }
            free(node->data);
            node->data = data;
            return node;
    ...
    if (next) {
        if (node->data) {
            if ((key != 0) && (mask != 0)) {
                ...
                glog(L_WARN, "patricia_delete: refusing to remove default/zero route '%s'\n", s_ip);
                return NULL;
            }
        }
    }
    ...
    if (next) {
        if (node->data) {
            if ((key != 0) && (mask != 0)) {
                ...
                glog(L_WARN, "patricia_delete128: refusing to remove default/zero route '%s'\n", s_ip);
                return NULL;
            }
        }
    }
