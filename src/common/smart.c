...
static void disk_dump_attributes(SkDisk *d, const SkSmartAttributeParsedData *a, void* userdata) {
	...
	metric_add_labels3("smart_attribute_stat_value", &current_value, DATATYPE_INT, 0, "device", device, "id", aid, "identificator", name);
	//puts("smart_attribute_stat_raw_value");
	metric_add_labels3("smart_attribute_stat_raw_value", &pretty_value, DATATYPE_INT, 0, "device", device, "id", aid, "identificator", name);
	//puts("smart_attribute_stat_worst");
	metric_add_labels3("smart_attribute_stat_worst", &worst_value, DATATYPE_INT, 0, "device", device, "id", aid, "identificator", name);
	//puts("smart_attribute_stat_threshold");
	metric_add_labels3("smart_attribute_stat_threshold", &threshold, DATATYPE_INT, 0, "device", device, "id", aid, "identificator", name);
	//puts("smart_attribute_stat_online");
	metric_add_labels3("smart_attribute_stat_online", &online, DATATYPE_INT, 0, "device", device, "id", aid, "identificator", name);
	//puts("smart_attribute_stat_prefailure");
	metric_add_labels3("smart_attribute_stat_prefailure", &prefailure, DATATYPE_INT, 0, "device", device, "id", aid, "identificator", name);
	//puts("======");
...
void ata_smart_parsing(SkDisk *d, char *device)
{
	...
	metric_add_labels("smart_total_offline_data_collection_seconds", &total_offline_data_collection_seconds, DATATYPE_UINT, 0, "device", device);
	...
	metric_add_labels("smart_self_test_execution_percent_remaining", &self_test_execution_percent_remaining, DATATYPE_UINT, 0, "device", device);
	...
	metric_add_labels("smart_short_and_extended_test_available", &short_and_extended_test_available, DATATYPE_INT, 0, "device", device);

	//printf("Start Self-Test Available: %d\n", spd->start_test_available);
	int64_t start_test_available = spd->start_test_available;
	metric_add_labels("smart_start_test_available", &start_test_available, DATATYPE_INT, 0, "device", device);

	//printf("Abort Self-Test Available: %d\n", spd->abort_test_available);
	int64_t abort_test_available = spd->abort_test_available;
	metric_add_labels("smart_abort_test_available", &abort_test_available, DATATYPE_INT, 0, "device", device);
	...
	metric_add_labels("smart_short_test_polling_minutes", &short_test_polling_minutes, DATATYPE_UINT, 0, "device", device);
	...
	metric_add_labels("smart_extended_test_polling_minutes", &extended_test_polling_minutes, DATATYPE_UINT, 0, "device", device);
	...
	metric_add_labels("smart_conveyance_test_polling_minutes", &conveyance_test_polling_minutes, DATATYPE_UINT, 0, "device", device);
	...
	if (ret >= 0)
		metric_add_labels("smart_disk_size", &size, DATATYPE_UINT, 0, "device", device);
		//printf("Size: %lu B\n", (unsigned long) (size));

	if (sk_disk_smart_get_bad(d, &value) >= 0)
		metric_add_labels("smart_bad_sectors", &value, DATATYPE_UINT, 0, "device", device);
		//printf("Bad Sectors: %llu\n", value);

	if (sk_disk_smart_get_power_on(d, &power_on) < 0)
		metric_add_labels("smart_power_on", &power_on, DATATYPE_UINT, 0, "device", device);
		//printf("Powered On: %llu\n", power_on);

	if (sk_disk_smart_get_power_cycle(d, &value) >= 0)
	{
		//printf("Power Cycles: %llu\n", (unsigned long long) value);
		metric_add_labels("smart_power_cycles", &value, DATATYPE_UINT, 0, "device", device);

		if (value > 0 && power_on > 0)
		{
			uint64_t diff = power_on/value;
			metric_add_labels("smart_average_powered_on_per_power_cycle", &diff, DATATYPE_UINT, 0, "device", device);
			//printf("Average Powered On Per Power Cycle: %llu\n", power_on/value);
		}
	}

	if (sk_disk_smart_get_temperature(d, &value) >= 0)
		metric_add_labels("smart_temperature", &value, DATATYPE_UINT, 0, "device", device);
		//printf("Temperature: %"PRId64"\n", value);

	SkSmartOverall overall;
	if (sk_disk_smart_get_overall(d, &overall) >= 0)
		metric_add_labels2("smart_overall", &vl, DATATYPE_INT, 0, "device", device, "status", sk_smart_overall_to_string(overall));
	...
	metric_add_labels2("smart_model", &vl, DATATYPE_INT, 0, "device", device, "model", ipd->model);
	metric_add_labels2("smart_serial", &vl, DATATYPE_INT, 0, "device", device, "serial", ipd->serial);
	metric_add_labels2("smart_firmware", &vl, DATATYPE_INT, 0, "device", device, "firmware", ipd->firmware);
