...
void tls_close(uv_handle_t* handle)
{
	extern aconf* ac;
	context_arg *carg = handle->data;
	if (ac->log_level > 2)
		printf("3: tls_close carg with addr %p(%p:%p) with key %s, hostname %s\n", carg, carg->socket, carg->connect, carg->key, carg->hostname);
...
void tls_timeout_timer(uv_timer_t *timer)
{
	extern aconf* ac;
	...
	uv_close((uv_handle_t*)carg->socket, tls_close);
}
...
void tls_on_read(uv_tls_t* tcp, ssize_t nread, const uv_buf_t *buf)
{
	...
	extern aconf* ac;
	if (ac->log_level > 2)
		printf("2r: tls_on_read carg with addr %p(%p:%p) with key %s, hostname %s\n", carg, carg->socket, carg->connect, carg->key, carg->hostname);
	if(nread < 0 || !buf || !buf->base)
	{
		if (carg)
			uv_close((uv_handle_t*)tcp, tls_close);
		else
	...
	{
		uv_tls_read(tcp, alloc_buffer, tls_on_read);
	}
	else
	{
...
void tls_on_write(uv_write_t* req, int status)
{
	if (status)
	{
		fprintf(stderr, "uv_tls_write error: %s\n", uv_strerror(status));
		free(req);
		//uv_close((uv_handle_t*)req->handle, tls_close);
	...
	if (ac->log_level > 2)
		printf("2w: tls_on_write carg with addr %p(%p:%p) with key %s, hostname %s\n", carg, carg->socket, carg->connect, carg->key, carg->hostname);
	carg->write_time_finish = setrtime();
	req->handle->data = carg;
	uv_tls_read(req->handle, alloc_buffer, tls_on_read);
	carg->read_time = setrtime();

	free(req);
}

void tls_on_connect(uv_connect_t* connection, int status)
{
	if ( status < 0 )
		return;
	...
	uv_tls_t* stream = connection->handle;
	connection->handle->data = carg;
	stream->data = carg;
	carg->connect_time_finish = setrtime();

	if (ac->log_level > 2)
		printf("1: tls_on_connect carg with addr %p(%p:%p) with key %s, hostname %s\n", carg, carg->socket, carg->connect, carg->key, carg->hostname);
	uv_tls_read(stream, alloc_buffer, tls_on_read);
	if (carg->write == 1)
	{
		uv_buf_t buffer = uv_buf_init(carg->mesg, strlen(carg->mesg));
		uv_write_t *request = malloc(sizeof(*request));
		request->data = carg;
		uv_tls_write(request, stream, &buffer, 1, tls_on_write);
	...
	else if (carg->write == 2)
	{
		...
		uv_tls_write(request, stream, carg->buffer, carg->buflen, tls_on_write);
		carg->write_time = setrtime();
	}
}

void tls_connect_do(void* arg)
{
	extern aconf* ac;
	...
	uv_tls_t *socket = carg->socket = malloc(sizeof(*socket));
	uv_tls_init(loop, socket);
	uv_tls_keepalive(socket, 1, 60);

	carg->tt_timer->data = carg;
	uv_timer_init(loop, carg->tt_timer);
	uv_timer_start(carg->tt_timer, tls_timeout_timer, 5000, 0);
	...
	connect->data = carg;
	uv_tls_connect(connect, socket, (struct sockaddr *)carg->dest, tls_on_connect);
...
//}
//
//void tls_on_resolved(uv_getaddrinfo_t *resolver, int status, struct addrinfo *res)
//{
//	extern aconf* ac;
...
//	resolver->data = carg;
//	int r = uv_getaddrinfo(loop, resolver, tls_on_resolved, addr, port, NULL);
//	if (r)
//	{
//		fprintf(stderr, "%s\n", uv_strerror(r));
//		return;
...
//}
//
//void do_tls_client_carg(context_arg *carg)
//{
//	extern aconf* ac;
//	uv_loop_t *loop = ac->loop;
//
//	uv_getaddrinfo_t *resolver = malloc(sizeof(*resolver));
//	resolver->data = carg;
//	int r = uv_getaddrinfo(loop, resolver, tls_on_resolved, carg->hostname, carg->port, NULL);
//	if (r)
//	{
//		fprintf(stderr, "%s\n", uv_strerror(r));
//		return;
