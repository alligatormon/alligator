...
void tls_on_tcp_read(uv_stream_t *stream, ssize_t nrd, const uv_buf_t *data)
{
...
int uv_tls_accept(uv_tls_t *t, uv_handshake_cb cb)
{
    ...
    uv_read_start((uv_stream_t*)(t->tcp_hdl), alloc_cb, tls_on_tcp_read);
    return rv;
}
...
void tls_tcp_on_close(evt_tls_t *tls, int status)
{
...
int uv_tls_close(uv_tls_t *strm,  uv_tls_close_cb cb)
{
    ...
    return evt_tls_close(strm->tls, tls_tcp_on_close);
}
...
int uv_tls_connect(uv_tls_t *t, uv_handshake_cb cb)
{
    ...
    //return uv_read_start((uv_stream_t*)(t->tcp_hdl), alloc_cb, tls_on_tcp_read);
    return uv_read_start((uv_stream_t*)(t->tcp_hdl), alloc_buffer, tls_on_tcp_read);
}
