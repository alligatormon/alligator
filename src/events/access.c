...
uint8_t check_ip_port(uv_tcp_t *client, context_arg *carg)
{
	...
	carglog(carg, L_DEBUG, "tcp client port %d\n", ntohs(in_caddr->sin_port));
	carglog(carg, L_DEBUG, "tcp client host %s\n", addr);
	return ip_check_access(carg->net_tree_acl, carg->net6_tree_acl, addr);
}
...
uint8_t check_udp_ip_port(const struct sockaddr *caddr, context_arg *carg)
{
	...
	carglog(carg, L_DEBUG, "udp client host %s\n", sender);
