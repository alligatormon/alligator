...
void nginx_upstream_check_handler(char *metrics, size_t size, client_info *cinfo)
{
	...
	while(i<size)
	{
		...
		metric_add_labels4("nginx_upstream_check_rise_count", &rise, DATATYPE_UINT, 0, "upstream", upstream, "server", server, "status", status, "type", type);
		metric_add_labels4("nginx_upstream_check_fall_count", &fall, DATATYPE_UINT, 0, "upstream", upstream, "server", server, "status", status, "type", type);
	}
}
