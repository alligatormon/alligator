...
void nvidia_smi_handler(char *metrics, size_t size, context_arg *carg)
{
	...
	for (uint64_t i = 0; row - metrics < size; ++i) {
		...
		char gpu_index[NVIDIA_SMI_MAX_LEN] = {0};
		...
		for (uint64_t j = 0; (field - dict < row_copy) && (row_copy != 0) && (j < 255); ++j) {
			...
			if (!i) {
				...
				if (!strcmp(columns[j], "uuid"))
				...
				else if (!strcmp(columns[j], "index"))
					strlcpy(gpu_index, str, sizeof(gpu_index));
					...
					metric_add_labels5(metric_name, &val, DATATYPE_UINT, carg, "name", gpu_name, "uuid", gpu_uuid, "serial", gpu_serial, "index", gpu_index, "version", str);
				}
					...
					uint64_t val = 1;
					nvidia_smi_metric_set(carg, metric_name);
					metric_add_labels5(metric_name, &val, DATATYPE_UINT, carg, "name", gpu_name, "uuid", gpu_uuid, "serial", gpu_serial, "index", gpu_index, "pstate", str);
				}
					...
					nvidia_smi_metric_set(carg, metric_name);
					metric_add_labels4(metric_name, &value, DATATYPE_DOUBLE, carg, "name", gpu_name, "uuid", gpu_uuid, "serial", gpu_serial, "index", gpu_index);
				}
			}
