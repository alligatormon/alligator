...
string *squid_comm_epoll_incoming_mesg(host_aggregator_info *hi, void *arg) {
	return string_init_add(gen_http_query(0, "cache_object://localhost/comm_epoll_incoming", NULL, hi->host, "alligator", hi->auth, 1, "1.0"), 0, 0);
}

string *squid_forward_mesg(host_aggregator_info *hi, void *arg) {
	return string_init_add(gen_http_query(0, "cache_object://localhost/forward", NULL, hi->host, "alligator", hi->auth, 1, "1.0"), 0, 0);
}

string *squid_fqdncache_mesg(host_aggregator_info *hi, void *arg) {
	return string_init_add(gen_http_query(0, "cache_object://localhost/fqdncache", NULL, hi->host, "alligator", hi->auth, 1, "1.0"), 0, 0);
}

string *squid_diskd_mesg(host_aggregator_info *hi, void *arg) {
	return string_init_add(gen_http_query(0, "cache_object://localhost/diskd", NULL, hi->host, "alligator", hi->auth, 1, "1.0"), 0, 0);
}
