...
static inline void uwsgi_metric_set(context_arg *carg, const char *metric_name)
{
	namespace_metric_family_set(NULL, carg, metric_name, METRIC_TYPE_GAUGE, "uWSGI stats exported metric value.");
}
...
void uwsgi_handler(char *metrics, size_t size, context_arg *carg)
{
	...
	uwsgi_metric_set(carg, "uwsgi_listen_queue");
	...
	uwsgi_metric_set(carg, "uwsgi_listen_queue_errors");
	...
	uwsgi_metric_set(carg, "uwsgi_signal_queue");
	...
	uwsgi_metric_set(carg, "uwsgi_load");
	...
	for (i = 0; i < locks_size; i++)
	{
		...
		json_object_foreach(locks_obj, key, value)
		{
			...
			uwsgi_metric_set(carg, metricname);
