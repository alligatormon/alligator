...
void couchbase_buckets_handler(char *metrics, size_t size, context_arg *carg)
{
	...
	for (uint64_t i = 0; i < bucket_num; i++)
	{
		...
		char *generated_query = gen_http_query(HTTP_GET, carg->query_url, bucket_nodes_query_uri->s, carg->host, "alligator", NULL, "1.0", carg->env, NULL, NULL);

		char *key = malloc(255);
		snprintf(key, 255, "(tcp://%s:%u)/%s", carg->host, htons(carg->remote_addr.sin_port), bucket_nodes_query_uri->s);
...
void couchbase_nodes_list(char *metrics, size_t size, context_arg *carg)
{
	...
	for (uint64_t i = 0; i < nodes_num; i++)
	{
		...
		for (uint64_t j = 0; j < subqueries_len; j++)
		{
			...
			char *generated_query = gen_http_query(HTTP_GET, carg->query_url, query_string->s, carg->host, "alligator", NULL, "1.0", carg->env, NULL, NULL);

			char *key = malloc(255);
			snprintf(key, 255, "(tcp://%s:%u)/%s", carg->host, htons(carg->remote_addr.sin_port), query_string->s);
