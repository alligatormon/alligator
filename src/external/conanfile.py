...
class Deps(ConanFile):
    ...
    def build(self):
        ...
        #self.run("cd external/libhv && ./configure && make -j4")
        self.run("cd external/libhv && ./configure --without-http-server --without-http-client --without-http --without-evpp && make -j4")
