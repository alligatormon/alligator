...
char *build_json_from_tokens(config_parser_stat *wstokens, uint64_t token_count)
{
	...
	for (uint64_t i = 0; i < token_count; i++)
	{
		...
		else if (wstokens[i].context)
		{
			...
			if (!context_json)
			{
				if (!strcmp(wstokens[i].token->s, "aggregate") || !strcmp(wstokens[i].token->s, "x509") || !strcmp(wstokens[i].token->s, "mtail") || !strcmp(wstokens[i].token->s, "vrl") || !strcmp(wstokens[i].token->s, "entrypoint") || !strcmp(wstokens[i].token->s, "query") || !strcmp(wstokens[i].token->s, "grok") || !strcmp(wstokens[i].token->s, "action") || !strcmp(wstokens[i].token->s, "probe") || !strcmp(wstokens[i].token->s, "lang") || !strcmp(wstokens[i].token->s, "cluster") || !strcmp(wstokens[i].token->s, "instance") || !strcmp(wstokens[i].token->s, "resolver") || !strcmp(wstokens[i].token->s, "scheduler") || !strcmp(wstokens[i].token->s, "threaded_loop") || !strcmp(wstokens[i].token->s, "tls_certificate") || !strcmp(wstokens[i].token->s, "tls_key") || !strcmp(wstokens[i].token->s, "tls_ca") || !strcmp(wstokens[i].token->s, "namespace") || !strcmp(wstokens[i].token->s, "log_channel") || !strcmp(wstokens[i].token->s, "enrichment_table"))
			...
			if ((!strcmp(context_name, "puppeteer") || !strcmp(context_name, "chromecdp")) &&
			     ...
			     strcmp(wstokens[i].token->s, "metricstransform")))
				{
					...
					else if (!strcmp(context_name, "entrypoint") && !strcmp(wstokens[i].token->s, "log_channel_raw"))
					{
						if (!log_channel_raw_entrypoint)
						{
							++i;
					...
					else if (!strcmp(context_name, "entrypoint") && !strcmp(operator_name, "format"))
					{
						if (!format_entrypoint)
						{
							++i;
							format_entrypoint = json_string(wstokens[i].token->s);
							json_array_object_insert(operator_json, "format", format_entrypoint);
						}
					}
				...
				else if (wstokens[i].argument)
				{
					...
					else if (!strcmp(context_name, "entrypoint") && !strcmp(operator_name, "log_channel_raw"))
					{
						if (!log_channel_raw_entrypoint)
						{
							log_channel_raw_entrypoint = json_string(wstokens[i].token->s);
