...
2. **Log ingestion path** (`entrypoint` with `handler mtail`):
   ...
   - Full variable export (for metric **TTL** refresh on idle series) runs on the first chunk and then at least every **`mtail_full_export_interval`** seconds; between those, only touched variables are exported (see [mtail configuration](configuration.md#mtail_full_export_interval))
